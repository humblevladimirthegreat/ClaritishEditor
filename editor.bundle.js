(() => {
  ...
  function buildAdviceDecorations(matches) {
    const marks2 = matches.map((match, index) => {
      const from = match[0];
  ...
  var adviceHoverTooltip = hoverTooltip((view, pos) => {
  ...
  var savedText = removeAdviceReferences(localStorage.getItem("browserpad") || "");
  ...
  var dateString = (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
  var daysVisited = localStorage.getItem("daysVisited").split(",");
  ...
  document.querySelector("#updated").textContent = new Date(document.lastModified).toLocaleString(void 0, {
    year: "numeric",
    month: "short",
    day: "numeric",
    hour: "numeric",
